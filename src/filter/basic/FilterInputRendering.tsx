...
import {ColumnDataType} from "../FilterModel";
...
export function RenderInput(props: RenderInputProps) {
    switch (props.columnType) {
        ...
        case "boolean":
            return RenderBoolean(props.availableBooleanOptions ?? [], props.fieldProps)
        case "autocomplete":
...
function RenderBoolean(availableBooleanOptions: BooleanValue[], valueState: StateProp<boolean>) {
    return (
        <Autocomplete
            disableClearable={true}
            options={availableBooleanOptions}
            getOptionLabel={(option) => option.label}
            value={valueState.value ? availableBooleanOptions[0] : availableBooleanOptions[1]}
            onChange={(_, newValue) => valueState.setValue(newValue.value)}
            renderInput={(params) => (
                <TextField {...params} sx={{width: '100%'}} label="Wartość" size="small"/>
            )}
            size="small"
            fullWidth
        />
